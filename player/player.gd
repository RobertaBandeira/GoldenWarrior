...
signal damage_collected(value: int)
...
func _ready():
	...
	damage_collected.connect(func(value: int):
		GameManager.damage_counter += 1
		)
...
func strength(amount: int) -> int:
	sword_damage += amount
	if sword_damage > max_damage:
		sword_damage = max_damage
	return sword_damage
